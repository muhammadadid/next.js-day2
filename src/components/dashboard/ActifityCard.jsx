...
const ActifityCard = ({item, getActifity}) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await axios.delete(`https://travel-journal-api-bootcamp.do.dibimbing.id/api/v1/delete-activity/${item?.id}`, config)
  ...
  return (
    ...
    <div className="relative">
      <img className="w-full transition-transform duration-300 transform h-52 hover:scale-110" src={item?.imageUrls} alt={item?.title} />
  ...
  C270,131.321,263.732,125.053,256,125.053z"
          ...
          <p className="mt-4 text-lg text-center">
            Delete <span className="font-bold">{item?.title}</span> actifity?
          </p>
